...
/********************************************/
var canvas = document.getElementById('canvas_effect_snow')
var ctx = canvas.getContext('2d')
var height = canvas.height = $('#map-preview').height()
var width = canvas.width =  $('#map-preview').width()
var collection = []
var num_drops = 1024 // number of drops
var gravity = 1 // gravity multiplier 
var windforce = 0 // yea i'd just leave this
var windmultiplier = 0.007 // this freaks out on large numbers
var maxspeed = 5 // this is so you never run too fast (it is a multiplier not raw)
var gutter = 0.001 // the percentage distance to travel off screen before wrapping
...
function draw_frame() {
    ...
    ctx.canvas.width = $('#map-preview').width();
    ctx.canvas.height = $('#map-preview').height();
...
function init() {
    ...
    window.onresize = function () {
        height = canvas.height =  $('#map-preview').height();
        width = canvas.width =  $('#map-preview').width();
    };
}
init();